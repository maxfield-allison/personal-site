...
//
// The privacy page for BOTH sites lives in probablyfine-site
// (src/pages/privacy.astro, linked from this site's footer as `outbound-privacy`),
// so a new event here means editing a file in the other repo. That is the whole
// reason this check exists on this side too: the page cannot enforce a promise
// about a repo it cannot see.
//
// 1. THE PRIVACY PAGE IS A PROMISE, NOT A DESCRIPTION. It
...
const DECLARED = new Set([
  'case-study-read',   // reached the end of a case study (read-tracker.js)
  'cta-resume',        // the home page's primary call to action
  'email-click',       // a mailto link
  'field-note-read',   // reached the end of a field note (read-tracker.js)
  'nav-menu',          // the mobile disclosure menu was opened
  'nav-resume',        // Resume, from the nav
  'outbound-bluesky',
  'outbound-facebook',
  'outbound-github',
  ...
  'outbound-probablyfine',
  'outbound-source',   // this site's own repo
  'resume-download',   // the PDF
  'resume-print',      // the print button, which produces no pageview
  'resume-read-online',// "Read it here", a same-page jump to #experience
  'skip-link',         // skip-to-content was used
]);

// Two families are built from content ids rather than written out, so they
// cannot be enumerated: one entry per project, and one per source link on a
// project. They are declared as prefixes instead. The prefix still pins the
// vocabulary - a typo like `casestudy-` is not matched and fails the build.
const DECLARED_PREFIXES = [
  'case-study-',       // opening a case study from a listing (per project id)
  'outbound-',         // a source link on a case study (per project id + label)
];
